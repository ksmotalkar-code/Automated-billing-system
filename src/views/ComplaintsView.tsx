...
export function ComplaintsView() {
  ...
  const handleDelete = async (c: Complaint) => {
    if (c.status !== 'Resolved') {
      showAlert("Action Not Allowed", "Only resolved complaints can be deleted.");
      return;
    }
    setConfirmConfig({
      isOpen: true,
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <div className="flex flex-wrap items-center gap-1 sm:gap-2 mt-0.5">
                        ...
                        {c.billStatus && (
                          <span className={`text-[10px] px-2 py-0.5 rounded-full font-bold ${c.billStatus.includes('Unpaid') ? 'bg-rose-50 text-rose-600 border border-rose-100' : 'bg-emerald-50 text-emerald-600 border border-emerald-100'}`}>
                            {c.billStatus}
                          </span>
                        )}
                      </div>
                  ...
                  {c.status === 'Pending' ? (
                    <div className="flex justify-end gap-2 pt-1">
                      ...
                      <button 
                        ...
                        className="px-4 py-2 bg-emerald-500 hover:bg-emerald-600 text-white text-xs font-bold rounded-xl shadow-lg shadow-emerald-500/20 transition-all flex items-center gap-1.5"
                      >
                         <MessageCircle className="w-4 h-4" /> Resolve & Notify WhatsApp
                      </button>
                    </div>
                  ) : (
