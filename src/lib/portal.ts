...
export const getPortalData = async (portalId: string): Promise<PublicPortalData | null> => {
  ...
  // Fallback: Check if portalId is a Customer ID, and generate portal data dynamically.
  // This is required when links are generated by the WhatsApp chatbot, bypassing the UI explicitly generating a 'public_portals' document for it.
  const custSnap = await getDoc(doc(db, 'customers', portalId));
  if (custSnap.exists()) {
    const customer = custSnap.data() as Customer;
    if (!customer.ownerId) return null;
    let settings: AppSettings = {} as AppSettings;
    const settingsSnap = await getDoc(doc(db, 'settings', customer.ownerId));
